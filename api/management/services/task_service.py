...
class TaskService(BaseCRUDService):
    ...
    @classmethod
    def retrieve_schedules(cls, responsible_id: int, team: list[int], start_at: datetime, end_at: datetime):
        ...
        responsible_tasks = cls.__query_tasks(user_id=responsible_id,
                                              ...
                                              end_date=end_date)
        schedules = [{"user_id": responsible_id,
                      "tasks": responsible_tasks}]

        team = [t for t in team if t != responsible_id]
        for user_id in team:
            user_tasks = cls.__query_tasks(user_id=user_id,
                                           start_date=start_date,
                                           end_date=end_date)
            schedules.append({"user_id": user_id,
                              "tasks": user_tasks})
